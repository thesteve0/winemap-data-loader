...
class DatabaseLoader:
    ...
    def setupDb(self, server, user, password, dbname):
        ...
        # does table exist
        tb_exists = "select exists(select relname from pg_class where relname='" + "wine-reviews" + "')"
        cur.execute(tb_exists)
        execute = cur.fetchone()[0]
        if not execute:
            # make table
            cur.execute(
                'create table wine-reviews(country VARCHAR, designation VARCHAR, points INT, price VARCHAR, province VARCHAR, region_1 VARCHAR, region_2 VARCHAR, variety VARCHAR, winery VARCHAR);')
            conn.commit()
        # copy csv
        f = open(r'/wine-data.csv', 'r')
        cur.copy_from(f, "wine_reviews", sep=',')
        conn.commit()
        cur.execute("select * from wine-reviews limit 100;")
        data = cur.fetchone()[0]
        f.close()
